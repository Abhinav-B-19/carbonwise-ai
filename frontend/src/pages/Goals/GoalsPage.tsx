  ...
  const loadGoalsAndSync = async () => {
    try {
      ...
      const [goalsResponse, historyResponse] = await Promise.all([
        api.get(`/api/goals?userKey=${userKey}`),
        ...
        goals.map(async (goal: any) => {
          ...
          switch (goal.goalType) {
            ...
            case "General Sustainability":
              currentValue = latest.carbonScore;

              status =
                currentValue >= goal.targetValue ? "Completed" : "Active";

              break;

            case "Reduce Total Emissions":
              currentValue = latest.totalEmission;
          ...
          const needsUpdate =
            currentValue !== goal.currentValue || status !== goal.status;
            ...
            >
              <div
                className="
                  ...
                  "
              >
                <CreateGoalForm onSubmit={createGoal} loading={loading} />
              </div>
            </div>
              ...
              {goals.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-slate-500">
                    Create your first sustainability goal to start tracking
                    progress.
                  </p>
                </div>
              ) : (
